...
function StartState:init()
    ...
    --Gerar um quadro de blocos so para o menu de iniciar(nao jogavel)
    for i = 1, 64 do
        table.insert(positions, gFrames['tiles'][math.random(18)][math.random(6)])
    end
    self.transitionAlpha = 0
    --Pausar jogo entre transiçoes de cena
    ...
    if not self.pauseInput then
        ...
        if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
            if self.currentMenuItem == 1 then
                --Passar para o estado de Inicio de jogo
                Timer.tween(1, {
...
function StartState:render()
    for y = 1, 8 do
        for x = 1, 8 do
            ...
            love.graphics.draw(gTextures['match3'], positions[(y - 1) * x + x], 
...
function StartState:drawOptions(y)
    ...
    self.drawTextShadow('Start', VIRTUAL_HEIGHT / 2 + 8)
